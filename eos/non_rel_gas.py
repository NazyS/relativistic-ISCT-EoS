...
class Non_Rel_Gas(Eq_of_state):
    ...
    def p_total(self, T, mu):
        return self.g * T * (self.m*T/(2*np.pi*hbar**2))**1.5 * np.exp((-self.m)/T) * np.cosh(mu/T)
    ...
    def entr_analyt(self, T, mu):
        p = self.p_eq(T, mu)
        return p/T*(5./2. - mu/T*np.tanh(mu/T) + self.m/T)
    ...
    def Xi(self, T, **kwargs):
        return derivative(lambda T: self.entropy(T, 0, **kwargs), T, **kwargs)/self.entropy(T, 0, **kwargs)
