...
class f_p_exc_vol(Eq_of_state):
    def __init__(self, q1=0.5, a1=0.06, q2=0.25, a2=0.01, p0=20.0, R=0.4, m=940.0):
    ...
    def f(self, p):
        return 1 / (
            1 + self.a1 * (p / self.p0) ** self.q1 + self.a2 * (p / self.p0) ** self.q2
        )
    ...
    def parameters_string(self):
        return "R = {:.2f} fm, $p_0$ = {:.2f} $MeV/fm^3$\n$q_1$ = {:.2f}, $a_1$ = {:.2f}, $q_2$ = {:.2f}, $a_2$ = {:.2f}".format(
            self.R, self.p0, self.q1, self.a1, self.q2, self.a2
        )
