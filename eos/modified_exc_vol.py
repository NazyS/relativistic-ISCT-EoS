...
class modified_exc_volume(Eq_of_state):
    ...
    Each parameter is list of size = num_of_components
    """
    ...
    def __init__(
        ...
        super().__init__(num_of_eq_in_eos=1, num_of_components=len(m))

        self.m = m

        # f(p) function parameters
        self.q1 = (
            q1 if len(q1) == self.num_of_components else q1 * self.num_of_components
        )
        self.a1 = (
            a1 if len(a1) == self.num_of_components else a1 * self.num_of_components
        )
        self.q2 = (
            q2 if len(q2) == self.num_of_components else q2 * self.num_of_components
        )
        self.a2 = (
            a2 if len(a2) == self.num_of_components else a2 * self.num_of_components
        )

        self.q3 = (
            q3 if len(q3) == self.num_of_components else q3 * self.num_of_components
        )
        self.a3 = (
            a3 if len(a3) == self.num_of_components else a3 * self.num_of_components
        )

        self.p0 = (
            p0 if len(p0) == self.num_of_components else p0 * self.num_of_components
        )
    ...
    def f(self, p, componet):
        return 1 / (
            1
            + self.a1[componet] * (p / self.p0[componet]) ** self.q1[componet]
            + self.a2[componet] * (p / self.p0[componet]) ** self.q2[componet]
            + self.a3[componet] * (p / self.p0[componet]) ** self.q3[componet]
        )
    ...
    def EoS(self, p, T, *mu):
        return sum(
            T
            * self.g[componet]
            * phi(T, self.m[componet])
            * np.exp((mu[componet] - self.b(componet) * self.f(p, componet) * p) / T)
            - p
            for componet in range(self.num_of_components)
        )
    ...
    def parameters_string(self, format="short"):
        if format == "full":
            ...
            string += (
                ", "
                + self.gen_str_for_array(self.q2, name="q_2", quantity="")
                + ", "
            ...
            string += ", " + self.gen_str_for_array(self.a1, name="a_1", quantity="")
            string += (
                ", "
                + self.gen_str_for_array(self.q2, name="q_2", quantity="")
                + ", "
            ...
            string = (
                self.gen_str_for_array(self.q3, name="q_3", quantity="")
                + ", "
                + self.gen_str_for_array(self.a3, name="a_3", quantity="")
            )
            return string
        else:
            print("Wrong format!")
