...
if __name__ == "__main__":
    ...
    G_TOTAL = 1770.0
    G_FERMION = 140.0  # approx 141 in article
    ...
    def search_for_low_m(m, T, mu_b):
        eos = Relativistic_ISCT(
            m=m, R=0.39, b=0.0, components=2, eos="ISCT", g=[G_FERMION, G_BOSON]
        )
        entr_per_bar_dens = eos.entropy(T, mu_b, 0.0) / eos.density_baryon(T, mu_b, 0.0)
        return entr_per_bar_dens - ENTR_TO_BAR_DENS_RATIO

    m, info, solved, msg = fsolve(search_for_low_m, 30.0, args=(T, mu_b), full_output=1)

    if solved != 1:
        print("T: {}\t mu_b: {}\t m: {}".format(T, mu_b, msg))
        m = np.nan
        entr_ratio = np.nan
        dev = np.nan
    else:
        print("T: {}\t mu_b: {}\t m: {}".format(T, mu_b, m))
        print(msg)
        print("info", info)
